...
pub fn webrtc_initialize(
    ...
    let peer_offer_func: Box<dyn FnMut(JsValue)> = Box::new(move |e: JsValue| {
        let rtc_session_type = RtcSdpType::Offer;
        let json_e = String::from(&js_sys::JSON::stringify(&e).unwrap());
        let sdp_str_index = json_e.find("sdp").unwrap();

       //Ony works as long as "sdp" is the last attribute in this JSON
        let rtc_sdp_string = (&json_e[(sdp_str_index + 6)..(json_e.len() - 2)]).replace("\\r\\n", "\r\n"); 
        let rtc_sdp_str = rtc_sdp_string.as_str();
        ...
        let mut session_description_init: RtcSessionDescriptionInit =
            RtcSessionDescriptionInit::new(rtc_session_type);

        session_description_init.sdp(rtc_sdp_str);
